...
interface MPISearchRequestBody {
  payerId: null;
  visitTypeId: null;
  recPerPage: number;
  mpii1: string;
...
interface MPISearchRequest {
  head: RequestHead;
  body: MPISearchRequestBody;
}
...
export default async function handler(
  ...
  const { mpi, customerSiteId = 1 } = req.body;
  ...
  // Calculate date range (last 2 days to today)
  ...
  try {
    ...
    const transformedPatients = data.body.Data.map(
      (appointmentData: any, index: number) => {
