...
export default async function handler(
  ...
  try {
    const { policyData, patientId, appointmentId, encounterId, payerId } =
      req.body;

    if (!policyData || !patientId) {
      return res
        .status(400)
        .json({ error: "Policy data and patient ID are required" });
    }
    ...
    const payload = {
      ...
      body: {
        ...
        networkId: cleanPolicyData.networkId
          ? typeof cleanPolicyData.networkId === "string"
            ? parseInt(cleanPolicyData.networkId, 10)
            : cleanPolicyData.networkId
          : null,
        ...
        priorityPatientApplicable:
          cleanPolicyData.priorityPatientApplicable ?? 0,
    ...
    console.log(
      "Sending payload to update endpoint:",
    ...
    const isSuccess =
      updateResult.head?.StatusValue === "Success" ||
      updateResult.head?.StatusValue === 200 ||
      updateResult.head?.StatusText === "Success";

    if (!updateResponse.ok || !isSuccess) {
