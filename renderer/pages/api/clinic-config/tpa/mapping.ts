...
async function handleGet(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!config || !config.insurance_id) {
        return res.status(404).json({ error: 'TPA mapping not found' })
    }
...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (hospital_insurance_mapping_id === undefined || insurance_id === undefined || insurance_type === undefined) {
        return res.status(400).json({ error: 'hospital_insurance_mapping_id, insurance_id, and insurance_type are required' })
    }
    if (!insurance_name || typeof insurance_name !== 'string') {
        return res.status(400).json({ error: 'insurance_name is required' })
    }
