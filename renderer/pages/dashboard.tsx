...
export default function DashboardPage() {
  ...
  const [expandedInsurance, setExpandedInsurance] = React.useState<Set<number>>(
    new Set(),
  );
  ...
  const handlePrefillSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        const uniquePatients = response.body.Data.reduce(
          (acc: PatientData[], current: PatientData) => {
            const existingPatient = acc.find(
              (p) => p.patient_id === current.patient_id,
            );
            if (!existingPatient) {
              acc.push(current);
            }
        ...
        console.log(
          `Deduplicated: ${response.body.Data.length} records → ${uniquePatients.length} unique patients`,
        );
