...
export type {
  ...
  MantysStatusResponse,
};

export const eligibilityKeys = {
  all: ["eligibility"] as const,
  history: (clinicId?: string) =>
    [...eligibilityKeys.all, "history", clinicId] as const,
  historyItem: (id: string) =>
    [...eligibilityKeys.all, "history", "item", id] as const,
  historyByTask: (taskId: string) =>
    [...eligibilityKeys.all, "history", "task", taskId] as const,
  active: (clinicId?: string) =>
    [...eligibilityKeys.all, "active", clinicId] as const,
  completed: (clinicId?: string) =>
    [...eligibilityKeys.all, "completed", clinicId] as const,
  byPatient: (patientId: string) =>
    [...eligibilityKeys.all, "patient", patientId] as const,
  byMPI: (mpi: string) => [...eligibilityKeys.all, "mpi", mpi] as const,
  byAppointment: (appointmentId: string) =>
    [...eligibilityKeys.all, "appointment", appointmentId] as const,
  taskStatus: (taskId: string) =>
    [...eligibilityKeys.all, "task", taskId] as const,
};
...
export function useEligibilityByAppointment(
  appointmentId: string | number,
  enabled = true,
) {
  return useQuery({
    queryKey: eligibilityKeys.byAppointment(appointmentId.toString()),
    queryFn: () => eligibilityHistoryApi.getByAppointmentId(appointmentId),
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: eligibilityKeys.history(variables.clinicId),
      });
      queryClient.invalidateQueries({
        queryKey: eligibilityKeys.active(variables.clinicId),
      });
    },
  });
}
...
export function useEligibilityPolling(
  ...
  useEffect(() => {
    ...
    if (data.status === "complete" && !hasCompletedRef.current) {
      ...
      if (patientMPI) {
        queryClient.invalidateQueries({
          queryKey: eligibilityKeys.byMPI(patientMPI),
        });
      }
